...
export default function OnPage() {
  ...
  return (<>
    ...
    <section className="py-12 bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary-light)]"><div className="container-page max-w-3xl text-center">
      ...
      <p className="text-sm text-white/80 mb-6 max-w-lg mx-auto">Probeer Amare ON — natuurlijke energie zonder crash.</p>
      <div className="flex flex-col sm:flex-row justify-center gap-3"><AffiliateCTA label="Bestel ON nu → €27,41/maand" product="on" variant="urgency" /></div>
    ...
    <section className="py-12 bg-white"><div className="container-page max-w-3xl text-center">
      <h2 className="text-xl md:text-2xl font-cormorant font-bold text-[var(--color-text)] mb-3">Je dagelijkse energie, zonder de crash</h2>
      <p className="text-sm text-[var(--color-text-muted)] mb-6 max-w-lg mx-auto">Amare ON — framboos-grapefruit shot met guarana en B-vitamines.</p>
      <div className="flex flex-col sm:flex-row justify-center gap-3"><AffiliateCTA label="Bestel Amare ON →" product="on" variant="primary" /></div>
      <p className="mt-4 text-[9px] text-[var(--color-text-muted)]">* Deze uitspraken zijn niet beoordeeld door de NVWA. Supplementen zijn geen vervanging voor een gevarieerd voedingspatroon en een gezonde levensstijl.</p>
    </div></section>
  </>);
}
