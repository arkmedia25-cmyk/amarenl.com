...
function buildTopicPrompt({ queueDoc, existingArticles, rejectedTopics, openPendingTopics, excludedKeywords }) {
  ...
  return [
    ...
    "",
    "=== PRIORITEITSVOLGORDE VOOR ONDERWERPKEUZE (belangrijkste regel) ===",
    "Als het document een sectie '24 AĞUSTOS 2026 — Anahtar Kelime Fırsat Araştırması' (of een latere,",
    "vergelijkbare 'Öncelik Kuyruğu'-sectie met dezelfde structuur) bevat, gebruik DIE als leidend, niet",
    "de oudere tabellen eronder. Binnen die sectie:",
    "  1. Kies UITSLUITEND uit de 'Öncelik 1' / 'A' tabel (bevestigde, niet-kannibaliserende boşluk).",
    "     Sla 'Öncelik 2' / 'B' (nog te verifiëren) alleen op als je zelf expliciet hebt bevestigd —",
    "     via de meegegeven bestaande-titels-lijst hieronder — dat het onderwerp echt nog niet bestaat.",
    "  2. Kies NOOIT een onderwerp uit de YMYL/laag-prioriteit-uitsluitingslijst (sectie 'D'), ook niet",
    "     als er verder niets anders lijkt te passen — sla die dag dan liever over.",
    "  3. Sorteer binnen de Öncelik 1-tabel op de 'Ads-Rekabet'-kolom, LAAGSTE eerst (Orta vóór Yüksek,",
    "     Yüksek vóór Çok Yüksek indien aanwezig) — dus begin bij het zwakst-concurrerende zoekwoord dat",
    "     nog niet geschreven is, niet automatisch bij het hoogste zoekvolume. Bij gelijke concurrentie:",
    "     hoogste volume wint.",
    "Bevat het document geen zo'n sectie (bv. bij een oudere staat van het bestand), val dan terug op:",
    "kies het beste nog-niet-geschreven onderwerp — hoogste zoekvolume × commerciële intentie die nog",
    "geen ⏳→✅ transitie heeft gehad. Kies er in alle gevallen precies één onderwerp.",
    "",
